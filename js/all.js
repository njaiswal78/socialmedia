...
$( ".scroll-top" ).click( function(e) {
  e.preventDefault();
    $( "html, body" ).animate({
        scrollTop: 0
    }, 1500, "easeInOutExpo"); //1200 easeInOutExpo
});
...
function submitForm2(){
    ...
    $.ajax({
        ...
        data: "name2=" + name + "&email2=" + email + "&subject2=" + subject + "&message2=" + message,
        success : function(text){
            if (text == "success"){
                formSuccess2();
            } else {
...
function formError2(){
    //Animation Phone Form when made error in validation
    $("#contact-form2").removeClass().addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
        $(this).removeClass();
    });
}
